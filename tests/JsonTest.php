...
class JsonTest extends MethodTest
{
    ...
    protected array $notAimedRoute = [
        '/api/activities', //Format multipart data required
        ...
        '/api/directory_countries', //No locale has been set and current locale is undefined
        ...
        '/api/directory_departments/{uuid}',// POST attribut problem (media)
    ...
    public function getFilters(string $path, string $method): void
    {
        try{
    ...
    {
        $json = file_get_contents(__DIR__ . '/API/openapi.json');
        $this->json = json_decode($json, true);

        $file = fopen("./tests/Errors_log/error.json", "w");
        fwrite($file, "");
        fclose($file);
        ...
        foreach ($this->json['paths'] as $pathCurrent => $methodAction) {
            if (in_array($pathCurrent, $this->notAimedRoute)) {
                continue;
            }
...
//                $file = fopen("./tests/EndPoint/".$compteur.'_'.$filename.'_'.$this->method.'_'."Test.php", "w");
//                fwrite($file, '<?php' . PHP_EOL);
//                fwrite($file, 'namespace App\Tests\EndPoint;' . PHP_EOL);
//                fwrite($file, 'use App\Tests\MethodTest;' . PHP_EOL);
//                fwrite($file, 'class '.$filename.'_'.$this->method.'_'.'Test extends MethodTest {' . PHP_EOL);
...
//                fwrite($file, '            $response = static::createClient()->request(\'GET\', $pathCut, [\'query\' => [\'deletedAt\'=>\'false\'], \'headers\' => [\'Accept\' => \'application/ld+json\', \'Authorization\' => \'Bearer \'. $token]]);' . PHP_EOL);
...
//                fwrite($file, '             static::createClient()->request($method, $path, [\'query\' => [\'deletedAt\'=>\'false\'], \'headers\' => [\'Accept\' => \''.$this->format.'\', \'Authorization\' => \'Bearer \'.$token], \'json\' => $data]);'. PHP_EOL);
